...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val leftWithRightFiltered = right.filterAcc(p, leftFiltered)
    if (p(elem)) leftWithRightFiltered.incl(elem)
    else leftWithRightFiltered
  }

  private def maxRetweeted(a: Tweet, b: Tweet): Tweet =
    if (a.retweets > b.retweets) a
    else b

  def mostRetweeted: Tweet = (left, right) match {
    case (l, r) if l.isEmpty && r.isEmpty => elem
    case (l, r) if l.isEmpty && !r.isEmpty => maxRetweeted(elem, r.mostRetweeted)
    case (l, r) if !l.isEmpty && r.isEmpty => maxRetweeted(elem, l.mostRetweeted)
    case (l, r) => maxRetweeted(elem, maxRetweeted(l.mostRetweeted, r.mostRetweeted))
  }
