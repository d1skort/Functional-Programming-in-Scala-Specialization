...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val normalizedWord = w filter (_.isLetter) toLowerCase
    val m = normalizedWord groupBy (ch => normalizedWord.count(_ == ch))
    (for {
      (occ, chars) <- m
      char <- chars
    } yield (char, occ)).toList.sorted
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap){
      case (acc, (char, occ)) =>
        acc(char) - occ match {
          case 0 => acc - char
          case newOcc => acc updated (char, newOcc)
        }
    }.toList.sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(sentenceOcc: Occurrences): List[Sentence] =
      if (sentenceOcc.isEmpty) List(Nil)
        ...
        sentence <- iter(subtract(sentenceOcc, comb))
      } yield word :: sentence
