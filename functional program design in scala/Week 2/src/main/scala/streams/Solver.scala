...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val more = for {
      ...
      (neighbor, neighborHistory) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
    } yield (neighbor, neighborHistory)
    more #::: from(more, explored ++ more.map{ case (block, _) => block })
  }
